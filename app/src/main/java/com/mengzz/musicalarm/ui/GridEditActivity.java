...
public class GridEditActivity extends BaseActivity implements EditContract.View {

    public static final String EDIT_DATA = "GRID_EDIT_DATA";
    public static final String DELETE_NUM = "GRID_DELETE_NUM";
    ...
    public static void startForResult(FragmentActivity activity, ArrayList <?
        ...
        Intent intent =
                new Intent(activity, GridEditActivity.class).putParcelableArrayListExtra(EDIT_DATA, value);
    ...
    private void init(ArrayList <EditItemInfo> editData) {
        BaseQuickAdapter adapter =
                new BaseQuickAdapter <EditItemInfo, BaseViewHolder>(R.layout.item_grid_edit,
                        editData) {
                    @Override
                    protected void convert(BaseViewHolder helper, EditItemInfo item) {
                        File file = new File(item.getInfo());
                        if (!file.exists()) {
                            return;
                        }
                        ImageView view = helper.getView(R.id.iv_edit_image);
                        Glide.with(GridEditActivity.this).load(file).into(view);
                        helper.addOnClickListener(R.id.iv_edit_image_del);
                    }
                };
        rvEdit.setLayoutManager(new GridLayoutManager(this, RemindImageActivity.SPAN_COUNT));
